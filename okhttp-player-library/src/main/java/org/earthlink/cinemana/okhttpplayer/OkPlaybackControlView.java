...
import static org.earthlink.cinemana.okhttpplayer.R.id.showHideSubs;

/**
 * A view to control video playback of an {@link ExoPlayer}.
 */
public class OkPlaybackControlView extends FrameLayout {

    private static final String TAG = OkPlaybackControlView.class.getSimpleName();
    ...
    private final ExpandableLayout settings_layout;
    ...
    private ImageButton incrementSubtitle;
    private ImageButton decrementSubtitle;
    ...
    public OkPlaybackControlView(Context context) {
        this(context, null);
    }

    public OkPlaybackControlView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public OkPlaybackControlView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        LayoutInflater.from(context).inflate(R.layout.ok_playback_control_view, this);
        ...
        settings_layout = (ExpandableLayout) findViewById(R.id.settings_layout);

        qualityLL = (LinearLayout) findViewById(R.id.qualityLL);

        incrementSubtitle = (ImageButton) findViewById(R.id.incrementSubs);
        decrementSubtitle = (ImageButton) findViewById(R.id.decrementSubs);

        incrementSubtitle.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                incrementFontSize();
            }
        });
        decrementSubtitle.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                decrementFontSize();
            }
...
//        player.seekToDefaultPosition(PlayerPrefs.with(getContext()).getLastSelectedQuality());
    ...
    public void show() {
        if (!isVisible()) {
            ...
            if (subtitleLayout != null ) {
                subtitleLayout.setAlpha(0.7f);
                subtitleLayout.setBottomPaddingFraction(0.4f);
            }
    ...
    public void hide() {
        if (isVisible()) {
            setVisibility(GONE);
            settings_layout.collapse(false);
            hideSystemBars();
            if (subtitleLayout != null ) {
                subtitleLayout.setAlpha(1f);
                subtitleLayout.setBottomPaddingFraction(0.05f);
            }
            if (visibilityListener != null) {
                visibilityListener.onVisibilityChange(getVisibility());
            }
...
//                        | View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION  // in portrait mode,
    ...
    private void updatePlayPauseButton() {
        ...
        playButton.setImageResource(
                playing ? R.mipmap.pause_button : R.mipmap.play_button);
    }


    public void setVideoData(VideoFile videoFile, MappingTrackSelector trackSelector) {
    ...
    public void setSubtitleLayout(SubtitleView subtitleLayout) {
        this.subtitleLayout = subtitleLayout;

        findViewById(R.id.subtitlesLL).setVisibility(VISIBLE);

        fontScale = PlayerPrefs.with(getContext()).getSubtitleFontSize();
        ...
        if (PlayerPrefs.with(getContext()).showSubtitles()) {
            ((ImageView) findViewById(R.id.showHideSubs))
                    .setImageDrawable(getResources().getDrawable(R.mipmap.show_subs));
        } else {
            ((ImageView) findViewById(R.id.showHideSubs))
                    .setImageDrawable(getResources().getDrawable(R.mipmap.hide_subs));
    ...
    private void decrementFontSize() {
        ...
        PlayerPrefs.with(getContext()).setSubtitleFontSize(fontScale);
    }
    ...
    private void toggleSubtitle() {
        ...
        if (trackSelector.getCurrentMappedTrackInfo().length > 2) {
            if (PlayerPrefs.with(getContext()).showSubtitles()) {
                trackSelector.setRendererDisabled(2, true);
                ((ImageView) findViewById(showHideSubs))
                        .setImageDrawable(getResources().getDrawable(R.mipmap.hide_subs));

                PlayerPrefs.with(getContext()).setShowSubtitle(false);
            } else {
                trackSelector.setRendererDisabled(2, false);
                ((ImageView) findViewById(showHideSubs))
                        .setImageDrawable(getResources().getDrawable(R.mipmap.show_subs));
                PlayerPrefs.with(getContext()).setShowSubtitle(true);
            }
        }
    }
    ...
    private void addQualities() {
        ...
        for (Map.Entry<VideoFile.Resolution, Quality> entry : videoFile.qualities.entrySet()) {
            ...
            TextView qualityTV = (TextView) inflater.inflate(R.layout.quality_item, null);
//            String qualityText = VideoFile.getQualityString(quality);

            qualityTV.setText(quality.name);
            qualityTV.setTag(quality);
            qualityTV.setOnClickListener(qualityChangeListener);

            setQualityTextStyle(qualityTV, false);
            qualityLL.addView(qualityTV);
        }
    ...
    View.OnClickListener qualityChangeListener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            ...
            Quality quality = (Quality) view.getTag();
            videoFile.wantedResolution = quality.resolution;
            highlightSelectedQuality();

            PlayerPrefs.with(getContext()).setLastSelectedQuality(quality);
            changeQuality(quality);
        }
    };

    private void highlightSelectedQuality() {
        for (int i = 0; i < qualityLL.getChildCount(); i++) {
            TextView qualityTextView = (TextView) qualityLL.getChildAt(i);
            Quality quality = (Quality) qualityTextView.getTag();

            if (quality.resolution == videoFile.wantedResolution) {
                setQualityTextStyle(qualityTextView, true);
            } else {
                setQualityTextStyle(qualityTextView, false);
            }
        }
    }

    private void setQualityTextStyle(TextView qualityTextView, boolean active) {
        qualityTextView
                .setBackground(getResources().getDrawable(
                        active
                                ? R.drawable.quality_back_selected
                                : R.drawable.quality_back_unselected));
    }

    public void changeQuality(Quality quality) {
        TrackGroupArray
                trackSelections = trackSelector.getCurrentMappedTrackInfo().getUnassociatedTrackGroups();
        ...
        player.seekToDefaultPosition(quality.windowIndex);
...
//                    || currentWindow.isDynamic;
        ...
        setButtonEnabled(enablePrevious, settingsButton);
//        setButtonEnabled(enableNext, nextButton);
    ...
    void showHideSettingsMenu() {
        settings_layout.toggle();
        if (subtitleLayout != null) {
            subtitleLayout.setBottomPaddingFraction(0.7f);
        }

//        player_controls_layout.toggle();
    }
